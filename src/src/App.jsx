...
export default function App() {
  ...
  useEffect(() => {
    ...
    const handle = (table, setRaw) => (payload) => {
    ...
    const tasksSub = sb.channel("tasks-changes")
      .on("postgres_changes", { event: "*", schema: "public", table: "tasks", filter: `user_id=eq.${userId}` }, handle("tasks", setTasksRaw))
      .subscribe();

    const rolesSub = sb.channel("roles-changes")
      .on("postgres_changes", { event: "*", schema: "public", table: "roles", filter: `user_id=eq.${userId}` }, handle("roles", setRolesRaw))
      .subscribe();

    const notesSub = sb.channel("notes-changes")
      .on("postgres_changes", { event: "*", schema: "public", table: "notes", filter: `user_id=eq.${userId}` }, handle("notes", setNotesRaw))
      .subscribe();

    return () => {
      sb.removeChannel(tasksSub);
      sb.removeChannel(rolesSub);
      sb.removeChannel(notesSub);
    };
  }, [userId]);
